...
impl DependencyUpdater {
    ...
    pub fn update_dependency(
        ...
        // Run cargo update to update Cargo.lock. The manifest now pins a
        // newer, incompatible version; --precise forces the exact resolve
        // rather than relying on cargo's default range search.
        self.update_lockfile(dep_name, new_version)?;
    ...
    /// Update Cargo.lock for the specific dependency, pinning to the exact
    /// requested version so incompatible bumps resolve cleanly.
    fn update_lockfile(&self, dep_name: &str, new_version: &str) -> Result<()> {
        let output = Command::new("cargo")
            .args(["update", "-p", dep_name, "--precise", new_version])
